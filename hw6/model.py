...
class VanillaRNNCell(nn.Module):
    ...
    def forward(self, prev_hidden: Tensor, cur_input: Tensor) -> Tensor:
        ...
        return torch.tanh(
            self.input2hidden(cur_input) + self.hidden2hidden(prev_hidden)
        )
...
class LSTMCell(nn.Module):
    ...
    def forward(
        ...
        f_t = torch.sigmoid(self.forget_linear(combined_input))       # Forget gate
        i_t = torch.sigmoid(self.input_linear(combined_input))        # Input gate
        g_t = torch.tanh(self.candidate_linear(combined_input))       # Candidate memory
        o_t = torch.sigmoid(self.output_linear(combined_input))       # Output gate

        # Update memory and hidden state
        c_t = f_t * prev_memory + i_t * g_t                           # New memory
