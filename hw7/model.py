...
class Seq2SeqModel(nn.Module):
    ...
    def forward(
        ...
        encoder_outputs, (hidden, cell) = self.encode(source_emb, source_lengths)

        source_padding_mask = self.get_padding_mask(
            batch_size=source.size(0),
            max_key_len=source.size(1),
            key_lengths=source_lengths
        ).type(self.embedding.weight.dtype)

        decoder_outputs, _ = self.decode(
            (hidden, cell), target_emb, target_lengths, encoder_outputs, source_padding_mask
        )
    ...
    def encode(
        ...
        packed = pack_padded_sequence(source_embeddings, source_lengths, batch_first=True, enforce_sorted=False)
    ...
    def decode(
        ...
        packed = pack_padded_sequence(target_embeddings, target_lengths, batch_first=True, enforce_sorted=False)
        packed_out, (h, c) = self.decoder(packed, inits)
        decoder_out, _ = pad_packed_sequence(packed_out, batch_first=True)

        attn_context = self.attention(decoder_out, encoder_states, padding_mask)
        combined = torch.cat([decoder_out, attn_context], dim=-1)
    ...
    def attention(
        ...
        scores = torch.matmul(decoder_states, encoder_states.transpose(1, 2))
        if padding_mask is not None:
            scores = scores + padding_mask
        weights = F.softmax(scores, dim=-1)
        return torch.matmul(weights, encoder_states)
