...
class Vocabulary:
    ...
    def __init__(
        ...
        # TODO: your code here!! (~5-6 lines)
        # add tokens from the frequency counter to `self.index_to_token`
        # popoulate self.index_to_token with tokens from `frequencies`
        # make sure to check that max_size has not been reached
        # and only include tokens that occur at least min_freq times
        sorted_tokens = sorted(frequencies.items(), key=lambda x: (-x[1], x[0]))
        for token, count in sorted_tokens:
            if count < min_freq:
             continue
            if max_size is not None and len(self.index_to_token)>=max_size:
             break
        ...
        # generate the reverse token --> index mapping, and store it in
        # `self.token_to_index`
        # this should be a dictionary, with tokens as keys, and indices as values
        self.token_to_index = {
    ...
    def tokens_to_indices(self, tokens: Iterable[str]) -> list[int]:
        ...
        # return a list of indices corresponding to an iterable of tokens
        return [self[token] for token in tokens]

    def indices_to_tokens(self, indices: Iterable[int]) -> list[str]:
        """Get all tokens for a list of integer indices. """
        # TODO: your code here!! (~ 1 line)
        # return a list of tokens (strings) corresponding to an iterable of indices
