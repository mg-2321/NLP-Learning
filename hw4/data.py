...
class SSTClassificationDataset(Dataset):
    ...
    def example_to_tensors(self, index: int) -> dict[str, np.ndarray]:
        ...
        bag_of_words = np.zeros(len(self.vocab), dtype=int)
        for word in example["review"]:
            index_in_vocab = self.vocab[word]
            bag_of_words[index_in_vocab] += 1

        #print("bag_of_words shape in example_to_tensors:", bag_of_words.shape)
        return {
            "review": bag_of_words.astype(float),
