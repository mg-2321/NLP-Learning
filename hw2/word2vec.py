...
class SGNS:
    ...
    def forward(self, example: tuple[int, int]) -> Result:
        ...
        w_idx, c_idx = example
        u_w = self.embeddings[w_idx]           
        v_c = self.context_embeddings[c_idx]   
        score = np.dot(u_w, v_c)
        prob  = sigmoid(score)
        return {
            "target_word_embedding": u_w,
            "context_word_embedding": v_c,
            "probability": prob,
        }
def get_positive_context_gradient(
    ...
    u_w = positive_result["target_word_embedding"]
    prob = positive_result["probability"]
    return (prob - 1) * u_w
...
def get_target_word_gradient(
    ...
    v_c_pos = positive_result["context_word_embedding"]
    sig_pos = positive_result["probability"]
    grad = (sig_pos - 1.0) * v_c_pos
    for neg in negative_results:
        sig_neg = neg["probability"]
        v_c_neg = neg["context_word_embedding"]
        grad   += sig_neg * v_c_neg
    return grad
