...
def bce_loss(probabilities: Tensor, labels: Tensor) -> Tensor:
    ...
    left = ops.multiply(labels, ops.log(probabilities))
    right = ops.multiply(ops.subtract(ones, labels), ops.log(ops.subtract(ones, probabilities)))
    summed = ops.add(left, right)
    mean = ops.reduce_mean(summed)
    return ops.multiply(minus_one, mean)
...
def dot_product_rows(mat1: Tensor, mat2: Tensor) -> Tensor:
    ...
    return ops.sum_along_columns(ops.multiply(mat1, mat2))
...
class Word2Vec(nn.Module):
    ...
    def forward(self, target_indices: Tensor, context_indices: Tensor) -> Tensor:
        ...
        target_vecs = self.embeddings.forward(target_indices)
        context_vecs = self.context_embeddings.forward(context_indices)
        dots = dot_product_rows(target_vecs, context_vecs)
        return ops.sigmoid(dots)
